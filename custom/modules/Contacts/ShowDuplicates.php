...
//$query = 'select id, first_name, last_name, title, primary_address_city, primary_address_state from contacts where deleted=0 '; 
$query = 'select contacts.id as "id", first_name, last_name, title, primary_address_city, primary_address_state, accounts.name as "account" 
from contacts inner join accounts_contacts on contacts.id = contact_id and accounts_contacts.deleted = 0 
...
if ($count > 0)
{
	...
	foreach ($duplicates as $duplicate_id) 
	{
		if (!$first) $query .= ' OR ';
		$first = false;
		$query .= "contacts.id='$duplicate_id' ";
	}
	$query .= ')';
}
