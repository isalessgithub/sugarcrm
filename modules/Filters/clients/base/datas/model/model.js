...
var filterList=[];var tokens=searchTerm.split(' ');for(var i=1;i<tokens.length;++i){var first=_.first(tokens,i).join(' ');var rest=_.rest(tokens,i).join(' ');_.each(fieldNames,function(name){filterList.push(this._buildFilterDef(name,operator,first));filterList.push(this._buildFilterDef(name,operator,rest));},this);}
