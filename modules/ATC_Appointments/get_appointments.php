...
//$GLOBALS['log']->fatal("++".$_REQUEST['contact_id']);
$GLOBALS['log']->fatal("--".$_REQUEST['target_list_id']);
...
$GLOBALS['log']->fatal($query_appointments_ids);

$GLOBALS['db'];
//$db = DBManagerFactory::getInstance();

//$result = $GLOBALS['db']->query($query_appointments_ids);
$result = $GLOBALS['db']->query($query_appointments_ids);

$GLOBALS['log']->fatal("mark 4");
...
while($row = $GLOBALS['db']->fetchByAssoc($result)){
	...
	// h,m,s,a
	$GLOBALS['log']->fatal("mark X:".$appointment->appointment_date);
if($appointment->appointment_date != '' && !empty($appointment->appointment_date)){
...
$GLOBALS['log']->fatal(json_encode($output));
