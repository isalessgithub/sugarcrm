...
if(!class_exists('OutfittersLicense1')) 
{
    class OutfittersLicense1
    {
        /**
        ...
        public static function isValid($thisModule=null,$user_id=null,$force_validation=false)
        {
            ...
            $validated = OutfittersLicense1::doValidate($thisModule);
            
            if (isset ($validated) && isset ($validated['result']) && 
                isset ($validated['result']['user_count']) && isset ($validated['result']['licensed_user_count'])) {
                    $user_count = $validated['result']['user_count'];
                    $licensed_user_count = $validated['result']['licensed_user_count'];
                    $validated['result']['validated_users'] = $licensed_user_count - $user_count >= 0 ? true : false;
            }
        ...
        public static function validate()
        {
            ...
            $validated = OutfittersLicense1::doValidate($currentModule,$_REQUEST['key']);
        ...
        public static function doValidate($thisModule,$key=null)
        {
            ...
            if(!empty($key)) 
            {
                ...
                $data = SugarOutfitters_API1::get_default_payload_static($thisModule);
            }
            ...
            foreach($api_urls as $api_url){
                $response = SugarOutfitters_API1::call($thisModule,'key/validate',$data,'get',$api_url);
                if($response['success']){
                    break;
                }
            }
            ...
            /*$response = SugarOutfitters_API1::call($thisModule,'key/validate',$data,'get','api_url');
            if(strlen($data['key']) != 35 && (empty($response['success']) || $response['success']!==true)){
                $response = SugarOutfitters_API1::call($thisModule,'key/validate',$data,'get','api_url1');
            }*/

            if(empty($response['success']) || $response['success']!==true) 
            {
                $GLOBALS['log']->fatal('OutfittersLicense1::doValidate() failed: '.print_r($response,true));
            }
        ...
        public static function change()
        {
            ...
            $response = SugarOutfitters_API1::call($currentModule,'key/change',$data);

            //if it is not a 200 response assume a 400. Good enough for this purpose.
            if(empty($response['success']) || $response['success']!==true) 
            {
                header('HTTP/1.1 400 Bad Request');
                $GLOBALS['log']->fatal('OutfittersLicense1::change() failed: '.print_r($response,true));
            } 
            else 
            {
                require_once('modules/Administration/Administration.php');
        ...
        public static function add()
        {
            ...
            $response = OutfittersLicense1::doValidate($currentModule);
...
if (!class_exists('SugarOutfitters_API1'))
{
    class SugarOutfitters_API1
    {
        ...
        public static function call($module,$path,$custom_data=array(),$method='post',$api_url)
        {
            ...
            if($info['http_code'] == 0)
            {
                $GLOBALS['log']->fatal('SugarOutfitters_API1::call(): Unable to validate license. Please configure the firewall to allow requests to '.$api_url.'/key/validate and make sure that SSL certs are up to date on the server.');
                return array(
                        'success' => false,
                        'result' => 'SugarOutfitters_API1::call(): Unable to validate the license key. Please configure the firewall to allow requests to '.$api_url.'/key/validate and make sure that SSL certs are up to date on the server.'
                    );
            }
            else if($info['http_code'] != 200)
            {
                $GLOBALS['log']->fatal('SugarOutfitters_API1::call(): HTTP Request failed: '.print_r($info,true));
                return array(
                    'success' => false,
                    'result' => $result
                );
